Shader "Hidden/Universal Render Pipeline/UberPost"
{
    HLSLINCLUDE
        ...
        float2 UVRotateAround(float2 uv, float2 center, float angle)
        {
            float u = (uv.x - center.x) * cos(angle) - (center.y - uv.y) * sin(angle) + center.x;
            float v = (uv.x - center.x) * sin(angle) + (center.y - uv.y) * cos(angle) + center.y;
            return float2(u, v);
        }
        ...
        half4 Frag(Varyings input) : SV_Target
        {
            ...
            #if defined(BLOOM)
            {
                ...
                #if defined(BLOOM_DIRT)
                {
                    ...
                    float angle_ =  3.14 * (1 +  range_ * sin(_Time.x*speed_));
	                uvDistorted = UVRotateAround(uvDistorted, float2(0.6, 0.8), angle_);
                    ...
                    //color = SAMPLE_TEXTURE2D(_BlueNoise_Texture, sampler_LinearRepeat, uvDistorted + float2(_Time.x * speed, 0));
                }
                #endif
            }
            #endif
