...
namespace BatterGame
{
    ...
    public class CharacterMoveControl
    {
        ...
        //1 5 8 8 10
        public CharacterMoveControl(float accelerationTimeAmount, float groundAccelerationTimeReduceFactor,
            float groundDeccelerationTimeReduceFactor, float airAccelerationTimeReduceFactor,
            float airDeccelerationTimeReduceFactor)
        {
        ...
        public void SetAccelerationLeftTimeNormalized(float normalizedTime) => AccelerationTimeStart =
            m_AccelerationTimeAmount == Mathf.Infinity ? 0 : m_AccelerationTimeAmount * (1 - normalizedTime);
        ...
        void CharacterAccelerating(bool isCharacterAccelerating, bool isGrounded)
        {
            if (isCharacterAccelerating)
            {
                ...
                {
                    AccelerationTimeLeft -= Time.deltaTime * (isGrounded
                        ? GroundAccelerationFactor * m_GroundAccelerationTimeReduceFactor
                        : AirAccelerationFactor * m_AirAccelerationTimeReduceFactor);
                    IsAccelerating = true;
                }
            ...
            else
            {
                ...
                {
                    AccelerationTimeLeft += Time.deltaTime * (isGrounded
                        ? GroundDecelerationFactor * m_GroundDecelerationTimeReduceFactor
                        : AirDecelerationFactor * m_AirDecelerationTimeReduceFactor);
                    IsDecelerating = true;
                }
