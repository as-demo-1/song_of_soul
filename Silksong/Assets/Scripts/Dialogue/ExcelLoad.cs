...
public class ExcelLoad : MonoBehaviour
{
    ...
    public static void ReadExcelStream(string FilePath)
    {
        ...
        Dictionary<int, string> Content = new Dictionary<int, string>(); //对话内容
        List<string> Condition = new List<string>();//存储条件
        Dictionary<int, List<string>> ConditionList = new Dictionary<int, List<string>>();//前面是对话ID（只要装每段对话的第一句对话的ID），后面是这个对话的条件列表
        Dictionary<int, List<int>> NPCID = new Dictionary<int, List<int>>(); //前面装NPCID，后面装这个NPCID的所有SID
        List<int> AllSID = new List<int>(); //给NPCID当工具人，装所有SID
        Dictionary<int, int> SidGetNpcID = new Dictionary<int, int>();//通过SID查找NPCID
        Dictionary<int, string> type = new Dictionary<int, string>();//前面是SID，后面是剧情对话类型
        Dictionary<int, string> NPCName = new Dictionary<int, string>(); //前面装NPC的SID，后面装NPCName
        Dictionary<int, List<SortedDictionary<int, DialogueSO>>> npcDialogueDic = new Dictionary<int, List<SortedDictionary<int, DialogueSO>>>();//把excel里面同一个NPCID的DialogueSO物体集合起来
        SortedDictionary<int, DialogueSO> dialoguesoDic = new SortedDictionary<int, DialogueSO>(); //前面是每个DialogueSO的StartID，用于按顺序装入SectionSO
        Dictionary<int, int> SIDList = new Dictionary<int, int>(); //前面储存SID，后面储存EID
        ...
        {
            byte[] SIDLength = System.Text.Encoding.ASCII.GetBytes(result.Tables[0].Rows[i][0].ToString());
            for (int j = 6; j < columns; j++)
            {
                string nvalue = result.Tables[0].Rows[i][j].ToString();
                if (j == 6)
                {
                    Content.Add(int.Parse(result.Tables[0].Rows[i][1].ToString()), result.Tables[0].Rows[i][3].ToString());//储存对话内容
                    if (SIDLength.Length != 0)
                    {
                        //AllSID.Add(int.Parse(result.Tables[0].Rows[i][0].ToString()));
                        NPCID[int.Parse(result.Tables[0].Rows[i][4].ToString())] = new List<int>();
                        NPCID[int.Parse(result.Tables[0].Rows[i][4].ToString())].Add(int.Parse(result.Tables[0].Rows[i][0].ToString()));//储存对话的npcid
                        NPCName.Add(int.Parse(result.Tables[0].Rows[i][0].ToString()), result.Tables[0].Rows[i][6].ToString());
                        SIDList.Add(int.Parse(result.Tables[0].Rows[i][0].ToString()), int.Parse(result.Tables[0].Rows[i][2].ToString()));
                        SidGetNpcID.Add(int.Parse(result.Tables[0].Rows[i][0].ToString()), int.Parse(result.Tables[0].Rows[i][4].ToString()));
                        type.Add(int.Parse(result.Tables[0].Rows[i][0].ToString()), result.Tables[0].Rows[i][5].ToString());
                    }
                ...
                if (j > 6)
                {
                    ...
                    if (array.Length != 0)
                    {
                        ...
                        ConditionList.Add(int.Parse(result.Tables[0].Rows[i][0].ToString()), Condition);
                    }
                }
                //Debug.Log(nvalue);
            }
        ...
        //创建SO物体
        string CreatePath = Application.dataPath + "/Scripts/Dialogue/ScriptableObject/DialogueItem/";
        foreach (KeyValuePair<int, int> item in SIDList)
        {
            /*FileInfo SOfile = new FileInfo(CreatePath + item.Key.ToString() + "-" + item.Value.ToString() + ".asset");
            SOfile.Create();*/
        ...
        //创建StatusSO物体
        string CreateStatusPath = Application.dataPath + "/Scripts/Dialogue/ScriptableObject/DialogueStatus/";
        foreach (KeyValuePair<int, int> item in SIDList)
        {
        ...
        //创建SectionSO物体
        string CreateSectionPath = Application.dataPath + "/Scripts/Dialogue/ScriptableObject/DialogueSectionItem/"; ;
        foreach (int item in NPCID.Keys)
        {
            if (!Directory.Exists(CreateSectionPath + NPCName[NPCID[item][0]] + "Section.asset"))
            {
                FileStream fs = new FileStream(CreateSectionPath + NPCName[NPCID[item][0]] + "Section.asset", FileMode.Create);
                fs.Write(System.Text.Encoding.Default.GetBytes("%YAML 1.1\n"), 0, System.Text.Encoding.Default.GetBytes("%YAML 1.1\n").Length);
                fs.Write(System.Text.Encoding.Default.GetBytes("%TAG !u! tag:unity3d.com,2011:\n"), 0, System.Text.Encoding.Default.GetBytes("%TAG !u! tag:unity3d.com,2011:\n").Length);
                fs.Write(System.Text.Encoding.Default.GetBytes("--- !u!114 &11400000\n"), 0, System.Text.Encoding.Default.GetBytes("--- !u!114 &11400000\n").Length);
                fs.Write(System.Text.Encoding.Default.GetBytes("MonoBehaviour:\n"), 0, System.Text.Encoding.Default.GetBytes("MonoBehaviour:\n").Length);
                ...
                fs.Write(System.Text.Encoding.Default.GetBytes("  NPCName: \n"), 0, System.Text.Encoding.Default.GetBytes("  NPCName: \n").Length);
                fs.Close();
            }
        }
        ...
        //把DialogueSO和DialogueStatusSO压入SectionSO
        foreach (DialogueSectionSO sectionitem in TalkSOManager.Instance.DialogueContainer.DialogueSectionList)
        {
            foreach (SortedDictionary<int, DialogueSO> dic in npcDialogueDic[sectionitem.NPCID])//用SID定位
            {
                ...
                foreach (KeyValuePair<int,DialogueSO> item in dic) //key是StartID
                {
                    ...
                    sectionitem.NPCName = NPCName[item.Key];
                }
