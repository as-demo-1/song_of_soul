...
public class Singing : MonoBehaviour
{
    ...
    bool IfSinging()
    {
        if (PlayerInput.Instance.Singing.Value>0)
        {
    ...
    {
        if (effectPos.Count == 0) return;
        if (effectNum + 1 < effectPos.Count&&transform.localScale.x > effectPos[effectNum].x && transform.localScale.x < effectPos[effectNum + 1].x )
        {
