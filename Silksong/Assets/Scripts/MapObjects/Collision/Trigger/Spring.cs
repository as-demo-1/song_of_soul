...
public class Spring : TwoTargetDamager
{
    ...
    protected override void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        else if (!IfSpikes && collision.GetComponent<PlayerController>())
        {
            if (!ifUp)
            {
                times = 0;
                collision.GetComponent<PlayerController>().RB.velocity = Vector2.zero;
                collision.GetComponent<PlayerController>().RB.AddForce(Force);
